...
def correspondenceMatricesTheta(theta, p2d, p3d, threshold):
    if theta.shape[1]==7:
        R = geo.quaternion_to_matrix(theta[..., 3:])
        t = theta[..., :3]
    else:
...
class TotalLossWithRep(torch.nn.Module):
    def __init__(self, gamma):
        super(TotalLossWithRep, self).__init__()
        self.gamma = gamma
        self.loss = torch.nn.BCELoss()
    def forward(self, theta, P, R_gt, t_gt, C_gt,  p2d, p3d, num_points_2d, num_points_3d):
        if self.gamma > 0.0:
...
class LossWithPandQuat(torch.nn.Module):
    def __init__(self, gamma):
        super(LossWithPandQuat, self).__init__()
        self.gamma = gamma
        # self.loss = torch.nn.BCELoss()
    def forward(self, theta, P, R_gt, t_gt, C_gt):
        if self.gamma > 0.0:
            ...
            losses = torch.cat((
                correspondenceLoss(P, C_gt).view(1),
                # correspondenceProbabilityBCE(P, C_gt).view(1),
                rotationLoss(R, R_gt).view(1), # [0, pi]
                translationLoss(t, t_gt).view(1), # [0, inf)
                ))
            loss = losses[0] + self.gamma * (losses[1] + losses[2])
            ...
            losses = correspondenceLoss(P, C_gt).view(1)
            # losses = correspondenceProbabilityBCE(P, C_gt).view(1)
            loss = losses[0]
        return loss, losses
