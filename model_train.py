...
# from model.cnnmodel import DBPnP as DeepPnP
...
parser = argparse.ArgumentParser(description='PyTorch GluePnP Training')
...
parser.add_argument('--start-epoch', default=10, type=int, metavar='N',
...
# modelnet40 先恢复原有顺序，然后选前n对数据，再打乱顺序
parser.add_argument('--sort', default=0, type=int, help='n number of subdata with re_sort')

"""
command:
Dataset Path  --dataset dataset_name(megadepth or modelnet40) --poseloss 12(Start using Lp) --gpu 0(Number of GPUs) 
--log-dir log_path --lr learning_rate
for example:  I://bpnpnet --dataset megadepth --poseloss 15 --gpu 0 --log-dir ./output  --lr 1e-4
"""
...
def main():
    ...
    def lr_lambda(epoch):
        if epoch < 12:
            ...
            return 1.0
            # 保持0.001 (lr * 1.0)
        elif epoch < 16:  # 第13-20个 epoch
            return 0.5
        elif epoch < 25:  # 第13-20个 epoch
            return 0.1  # 0.001 * 0.5 = 0.0005
        else:  # 25 epoch后
            return 0.9 ** (epoch - 25)  # 指数衰减
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch < 10:  # epoch < 10 dont validate loss, because its too slow
            error = (10 - epoch)*0.5
        else:
    ...
    for batch_index, (p2d, p3d, R_gt, t_gt, C_gt, num_points_2d, num_points_3d) in enumerate(train_loader):
        ...
        if args.frac_outliers == 0.0:
            ...
            for i in range(p2d.size(0)):
                ...
                num_outliers = int(args.frac_outliers * p2di.size(-2))
        ...
        for i in range(C_gt.size(0)):  # 用于数据长度小于设定长度时，将重复项匹配矩阵后面的数据置为0
        ...
        # Compute output
        P, theta_epro, theta_weightede = model(p2d, p3d, num_points_2d, num_points_3d, pose_gt, poseloss)
...
def test(val_loader, model, criterion, epoch, args):
    ...
    with torch.no_grad():
        ...
        for batch_index, (p2d, p3d, R_gt, t_gt, C_gt, num_points_2d, num_points_3d) in enumerate(val_loader):
            ...
            thetas0 += [theta_weighted[0, 0].item(), theta_weighted[0, 1].item(), theta_weighted[0, 2].item(),
                        theta_weighted[0, 3].item(),
                        theta_weighted[0, 4].item(), theta_weighted[0, 5].item()]
            thetas += [theta_epro[0, 0].item(), theta_epro[0, 1].item(), theta_epro[0, 2].item(),
                       theta_epro[0, 3].item(),
        ...
        print('Rotation_weighted Error: 25th percentile: {:.4f}, 50th percentile: {:.4f}, 75th percentile: {:.4f}'.
              format(rotation_percentage[0] * 57.2958, rotation_percentage[1] * 57.2958, rotation_percentage[2] * 57.2958))
        rotation_percentage1 = np.percentile(rotation_errors, [25, 50, 75])
        print('Rotation Error: 25th percentile: {:.4f}, 50th percentile: {:.4f}, 75th percentile: {:.4f}'.
              format(rotation_percentage1[0] * 57.2958, rotation_percentage1[1] * 57.2958, rotation_percentage1[2] * 57.2958))
        translation_percentage0 = np.percentile(translation_errors0, [25, 50, 75])
        print('Translation_weighted Error: 25th percentile: {:.4f}, 50th percentile: {:.4f}, 75th percentile: {:.4f}'.
              format(translation_percentage0[0], translation_percentage0[1], translation_percentage0[2]))
        ...
        print('Reprojection_weighted Error: 25th percentile: {:.4f}, 50th percentile: {:.4f}, 75th percentile: {:.4f}'.
              format(reprojection_percentage[0] * 57.2958, reprojection_percentage[1] * 57.2958, reprojection_percentage[2] * 57.2958))
        reprojection_percentage1 = np.percentile(reprojection_errors, [25, 50, 75])
        print('Reprojection Error: 25th percentile: {:.4f}, 50th percentile: {:.4f}, 75th percentile: {:.4f}'.
              format(reprojection_percentage1[0] * 57.2958, reprojection_percentage1[1] * 57.2958, reprojection_percentage1[2] * 57.2958))
        ...
        with open('./results/' + dataset_string + '/' + loss_string + '/results' + append_string + '.txt',
                  'w') as save_file:
            save_file.write(
                "rotation_errors0, rotation_errors, translation_errors0, translation_errors, reprojection_errors0, "
                "reprojection_errors, reprojection_errorsGT, num_inliers0, num_inliers, num_inliersLM, num_inliersGT, "
                "num_points_2d, num_points_3d, inference_time\n")
