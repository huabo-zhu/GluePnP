...
# 直接用旋转矩阵求重投影
def transform_and_normalise_points_by_Rt(p, R, t):
    pts_transformed = torch.einsum('brs,bms->bmr', (R, p))
    pts_transformed = pts_transformed + t.unsqueeze(-2)
    pts_transformed_normal = F.normalize(pts_transformed, p=2, dim=-1)
    return pts_transformed_normal
